...
# Appends one JSONL line per invocation to state/skill-uses.jsonl; `loopctl skill-stats` aggregates
...
cat 2>/dev/null | jq -c '
  if (.hook_event_name=="PostToolUse" and .tool_name=="Skill") then
    {ts:(now|todate), skill:(.tool_input.skill // .tool_response.commandName // ""), via:"model",
     args:(.tool_input.args // ""), session:(.session_id // ""), cwd:(.cwd // ""), agent:(.agent_id // "")}
  elif (.hook_event_name=="UserPromptExpansion") then
    {ts:(now|todate), skill:(.command_name // ""), via:"user",
     args:(.command_args // ""), session:(.session_id // ""), cwd:(.cwd // ""), agent:(.agent_id // "")}
  else empty end
  | select(.skill != "")
