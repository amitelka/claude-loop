...
- **Gardener** (Opus, daily): dedups/prunes/re-verifies against live code, enforces the `MEMORY.md` size cap. Never auto-edits skills. A run counts as done only if it wrote a fresh digest with no API error; a **missed** run (Mac asleep past 03:00) or a **failed** one self-heals via a catch-up — fired from the nightly harvest *and*, decoupled from launchd's once-a-day wake, the moment you're next active at the machine (`Stop`/`SessionStart` hook), gated to at most once / 2h and serialized by a store lock. `loopctl doctor` flags a garden stale >48h or a pending failed run.
