...
- **Triggers** — review at **session close** (`SessionEnd`) and a daily **harvest** backstop; a mid-session top-up per ~N tool-calls is an **opt-in knob** (`REVIEW_EVERY_TOOLCALLS`, default `0`) — the rung-2 triage insertion point, off by default. Watermarks advance only on a *successful* review, so a failed/API-errored run retries.
