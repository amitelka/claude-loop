...
vr(){ validate_store "$memdir" "${1:-}" 2>/dev/null; }
...
# drop tripwire (needs pre_rev): commit baseline, drop, validate against pre
build_store; pre="$(mhead)"; drop_slug c1; drop_slug c2; drop_slug c3; drop_slug h4   # 4 drops (>3); c4 + h1-3 keep both tiers non-empty
  ok "$(has "$(vr "$pre")" too-many-drops)" yes "drop tripwire: >GARDEN_MAX_DROPS drops → FAIL"
build_store; pre="$(mhead)"; drop_slug h1; ok "$(has "$(vr "$pre")" rule-typed-drop)" yes "rule-typed (feedback) drop → FAIL"
build_store; pre="$(mhead)"; drop_slug c1; ok "$(vr "$pre" >/dev/null; echo $?)" 0 "normal single reference prune → PASS"
